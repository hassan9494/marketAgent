    ...
    <div id="accordion" class="accordion-service">
        @forelse($services as $key => $service)
            <div class="card card-primary m-0 m-md-4 my-1 m-md-0">
                <div class="card-header" id="faqhead{{slug($key)}}">
                    <a href="#" class="btn btn-header-link" data-toggle="collapse"
                       data-target="#faq{{slug($key)}}" aria-expanded="true"
                       aria-controls="faq{{slug($key)}}">
                        @lang($key)
                    </a>
                </div>

                <div id="faq{{slug($key)}}" class="collapse @if($loop->first) show @endif"
                     aria-labelledby="faqhead{{slug($key)}}" data-parent="#accordion">
                    <div class="card-body">
                                    ...
                                    <tr>
                                        ...
                                        <td data-label="@lang('Name')" class="text-right text-lg-left">
                                            <a href="javascript:void(0)" data-container="body" data-toggle="popover"
                                               data-placement="top" data-content="{{$row->service_title}}">
                                        ...
                                        <td data-label="@lang('Status')">
                                            <span
                                                class="badge badge-pill {{ $row->service_status == 0 ? 'badge-danger' : 'badge-success' }}">{{ $row->service_status == 0 ? 'Inactive' : 'Active' }}</span>
                                        </td>


                                        <td data-label="@lang('Action')">
                                            <a href="{{route('admin.service.edit',['id'=>$row->id])}}"
                                            ...
                                            @if($row->service_status == 0)
                                                <a href="javascript:void(0)"
                                                   ...
                                                   data-route="{{route('admin.service.status.change',['id'=>$row->id])}}">
